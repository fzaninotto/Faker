...
### `Faker\Provider\User`
    ...
    // Get a random user from https://randomuser.me
    $user = $faker->user()
    
    $user->name->title;        // Title (mr, ms)
    $user->name->first;        // First name
    $user->name->last;         // Last name
    $user->location->street;   // Street address
    $user->location->city;     // City name
    $user->location->state;    // State name
    $user->location->postcode; // Postal code
    $user->email;              // Email
    $user->login->username;    // Username
    $user->login->password;    // Random password
    $user->login->salt;        // Random password salt
    $user->login->md5;         // Given password md5
    $user->login->sha1;        // Given password sha1
    $user->login->sha256;      // Given password sha256
    $user->dob;                // Date of Birth
    $user->registered;         // Date of registration
    $user->phone;              // Phone number
    $user->cell;               // Cell phone number
    $user->id->name;           // Identification
    $user->id->value;          // ID value
    $user->picture->large;     // Avatar pic, large
    $user->picture->medium;    // Avatar pic, medium
    $user->picture->thumbnail; // Avatar pic, thumbnail
