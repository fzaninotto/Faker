...
class Company extends \Faker\Provider\Company
{
    ...
    // CI-citycode-creation year-period-9587
    protected static $rccmPrefix = array('CI');
    /*
    * Côte d'Ivoire company registering number
    */
    protected static $rccmCityCode = array('ABJ', 'BKE', 'BKO','KGO','GRDSBM');
    protected static $rccmPeriod = array('B');
    protected static $rccmOderFormat = '####';
    /**
    * Generates rccm following data on https://www.cepici.ci/?tmp=annonces_legales&p=annonces-legales
    *
    * @return string
    */
    public function rccm($formated = true)
    {
        $prefix = static::randomElement(static::$rccmPrefix);
        $city =  static::randomElement(static::$rccmCityCode);
        $period =  static::randomElement(static::$rccmPeriod);
        $order = static::numerify(static::$rccmOderFormat);
        $year = static::numberBetween(1960,date('Y'));
        if($formated){
            return $prefix.'-'.$city.'-'.$year.'-'.$period.'-'.$order;
        }
        return $prefix.$city.$year.$period.$order;
    }
